...
- **`dispatcher`** — polls the DB for `queued` tasks, spawns **one fresh worker session per task**, releases crashed/timed-out claims back to the queue. Config: `dispatcher.config.json`. The worker engine is `claude -p` by default; set `engine`/`stageEngines` (e.g. `{ "stageEngines": { "implementation": "codex" } }`) to run a stage on **codex** instead. Claude reaches the board's MCP server via `--mcp-config`; codex has no such flag, so the dispatcher injects the `agentfactory` MCP server per-task via `codex exec -c mcp_servers.agentfactory.*` overrides (`src/codex.ts`) and feeds the worker prompt on stdin. Codex must be installed (or `AGENTFACTORY_CODEX_BIN`) and `codex login`-ed; its posture is `config.codexArgs` (default `--dangerously-bypass-approvals-and-sandbox`). On Windows the codex `.cmd` shim is bypassed (`node <codex.js>` spawned directly) so the `-c` TOML quotes survive.
...
Each supervisor still boots from its JSON file, but its **tunable knobs are board-editable** and read live: the "Supervisors" modal writes a sparse per-kind override into `app_kv` (`packages/core/src/supervisorSettings.ts`), and each supervisor re-resolves `fileConfig` + those DB settings at the top of every `tick()` (`resolveSupervisorConfig`) — so a change (concurrency, engine/`stageEngines`, poll, timeouts, `excludeWorkspaces`, `captureBuildErrors`, …) applies on the next poll with **no restart**. The DB path and secrets (`otel.token`, the `*Env` auth pointers) stay in the file/env and are never in settings.
