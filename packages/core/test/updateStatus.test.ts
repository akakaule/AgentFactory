...
describe('updateStatus', () => {
  ...
  it('a retry that started during repair accepts a concurrent merged completion', () => {
    ...
    db.prepare(
      `INSERT INTO task_delivery
        (task_id, provider, branch, pr_url, pr_id, pr_state, checks_state, detail, checked_at, state_changed_at, created_at, updated_at)
       VALUES (?, 'github', 'feature/t', 'https://github.com/acme/t/pull/1', '#1', 'merged', 'pending', '{}', ?, ?, ?, ?)`,
    ).run(task.id, FIXED_TS, FIXED_TS, FIXED_TS, FIXED_TS);

    // The initial read in updateStatus was made while this repair was blocked; the watcher won
    // the race and committed done before the retry's write transaction re-read the row.
    const exec = db.exec.bind(db);
    Object.defineProperty(db, 'exec', {
      value: (sql: string) => {
        if (sql === 'BEGIN IMMEDIATE') db.prepare("UPDATE task SET status='done' WHERE key=?").run(task.key);
        return exec(sql);
      },
    });
    const detail = updateStatus(db, task.key, 'queued', 'human', fixedNow);
    ...
    expect(detail.status).toBe('done');
    expect(detail.delivery).toMatchObject({ prState: 'merged', checksState: 'pending', prId: '#1' });
  });
