...
const MIGRATIONS: Migration[] = [
  ...
  // #21 adds task-level auto-review loop controls. Guard each ADD COLUMN for rewound/diverged DBs.
  (db) => {
    const cols = (db.prepare("PRAGMA table_info('task')").all() as Array<{ name: string }>).map((c) => c.name);
    if (!cols.includes('auto_review_enabled') && !cols.includes('auto_review_rounds')) db.exec(MIGRATION_21_SQL);
    else {
      if (!cols.includes('auto_review_enabled')) db.exec('ALTER TABLE task ADD COLUMN auto_review_enabled INTEGER NOT NULL DEFAULT 0;');
      if (!cols.includes('auto_review_rounds')) db.exec('ALTER TABLE task ADD COLUMN auto_review_rounds INTEGER NOT NULL DEFAULT 0;');
    }
  },
];
