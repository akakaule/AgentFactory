...
export function deleteAttachment(db: DB, id: number, now: () => string = nowIso): void {
  ...
  const task = db.prepare('SELECT status FROM task WHERE id = ?').get(row.taskId) as { status: Status } | undefined;
  if (task && !isBriefEditable(task.status))
    throw new InvalidTransitionError(`attachments can only change while a task is in backlog or queued`);
