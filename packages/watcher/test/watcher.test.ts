...
describe('watcher tick', () => {
  ...
  it('merged with checks still pending → done with pending checks retained', async () => {
    const core = makeCore();
    const key = deliverTask(core);
    const fetchJson = fakeFetch([
      ['/pulls?head=', { body: [ghPr({ merged_at: '2026-01-01T00:00:00Z', state: 'closed' })] }],
    ...
    const w = new Watcher(makeConfig(), makeDeps(core, fetchJson));
    await w.tick();
    expect(core.getTask(key)).toMatchObject({ status: 'done', delivery: { checksState: 'pending' } });
  });
