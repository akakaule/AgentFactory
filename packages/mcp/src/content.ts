import { isAiReviewMarker, isCurationMarker } from '@agentfactory/core';
 ...
 *
 * `ai-review/v1` and `curation/v1` comments are stripped from the activity handed to the
 * agent: neither the uncurated reviewer findings nor the human's forward/dismiss/override
 * judgment of them must leak into the implementer's brief — only the human-curated `feedback`
 * activity rides the re-claim. (Marker presence alone strips, so a malformed one can't slip
 * through.) The board UI keeps them; this filter is MCP-only.
 */
export function detailContent(core: Core, task: TaskDetail, extra?: Record<string, unknown>): Block[] {
  const activity = task.activity.filter(
    (a) => !(a.type === 'comment' && (isAiReviewMarker(a.body) || isCurationMarker(a.body))),
  );
