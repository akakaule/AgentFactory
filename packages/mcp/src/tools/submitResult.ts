  ...
  server.registerTool(
    'submit_result',
    {
      ...
      description:
        ...
        'Finish protocol — run it before calling this tool: (1) commit all work in the worktree, (2) `git push -u origin feature/<task-key>-<kebab-title>` so a PR can be opened from the pushed branch, (3) remove the worktree from the main repo: `git worktree remove <repoPath>/.worktrees/<task-key>` then `git worktree prune` (remove refuses on uncommitted changes — that is your signal something is not committed). The branch is the durable record of the work; the worktree must not outlive the submission. ' +
