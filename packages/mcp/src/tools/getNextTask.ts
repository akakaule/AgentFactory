...
export function registerGetNextTask(server: McpServer, core: Core, opts: ServerOptions = {}): void {
  server.registerTool(
    'get_next_task',
    {
      title: 'Get next task',
      description:
        'Claim the next queued task and move it to In Progress. Returns the full task detail — including its workspace and the repoPath of the repository the task targets — or { task: null } if the queue is empty. Spec images attached to the task arrive as image content blocks after the JSON; treat them as part of the brief. ' +
