...
export function buildWorkerPrompt(): string {
  return [
    ...
    'the deliverable this claim wants: a feature description, an implementation plan, or code.',
    ...
    'implement (TDD); `npm test` and `npm run build` must both be green from the worktree root and',
    'commits use Conventional Commits; then run the full `protocol.finish` sequence (commit all, push',
    'the feature branch, remove the worktree, prune) and call `submit_result` with a summary covering',
    'what you built, how each acceptance criterion is met, and what you verified, plus a branch link.',
